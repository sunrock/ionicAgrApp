...
const errorMessages = {
  ...
  invalidCharsName: Validation.profileNameValidator.patternError,
  nameTooShort: Validation.profileNameValidator.lengthError,
  changeEmail: { title: 'Change Email Failed!', subTitle: 'Sorry, but we\'ve encountered an error changing your email address.' },
  invalidProfileEmail: Validation.profileEmailValidator.patternError,
  changePhoto: { title: 'Change Photo Failed!', subTitle: 'Sorry, but we\'ve encountered an error changing your photo.' },
  passwordTooShort: Validation.profilePasswordValidator.lengthError,
  invalidCharsPassword: Validation.profilePasswordValidator.patternError,
  ...
  showEmailVerifiedMessageAndRedirect(navCtrl) {
    this.alert = this.alertCtrl.create({
      ...
      buttons: [{
        text: 'OK',
        handler: () => {
          navCtrl.setRoot(CusConfig.homePage);
        }
      }]
    }).present();
  }
